...
expected = {
    ...
    "99.06": {"C64_DCD"}, "104.14": {"C64_CTS"}, "106.68": {"C64_DSR"},
    "109.22": {"C64_TXD"}, "111.76": {"GND"},
}
for y, want in expected.items():
    got = set(labels_at("33.02", y))
    if got != want:
        raise SystemExit(f"M2.3 FAIL: J1 coordinate 33.02,{y}: expected {sorted(want)}, got {sorted(got)}")
